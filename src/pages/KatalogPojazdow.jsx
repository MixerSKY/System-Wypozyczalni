...
export default function KatalogPojazdow() {
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/api/Samochody/dostepne`)
      .then(response => {
  ...
  useEffect(() => {
    if (selectedCar) {
      axios.get(`${import.meta.env.VITE_API_URL}/api/Wypozyczenia/samochod/${selectedCar.id}/zajete-daty`)
  ...
  const handleRezerwacja = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/api/Wypozyczenia`, payload);

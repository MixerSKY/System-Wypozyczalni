...
export default function AdminWypozyczeniaPage() {
  ...
  const fetchWypozyczenia = () => {
    setLadowanie(true);
    axios.get(`${import.meta.env.VITE_API_URL}/api/Wypozyczenia/admin/wszystkie`)
      .then(response => {
  ...
  const handleStatusChange = async (wypozyczenieId, nowyStatusId) => {
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}/api/Wypozyczenia/${wypozyczenieId}/status/${nowyStatusId}`);
